...
import { Button, Grow, Card, CardActionArea, CardActions, CardContent, CardMedia, Typography } from '@material-ui/core';
    ...
    <Card className={classes.root}>
      <CardActionArea>
        <CardMedia 
          ...
          style={{background: 'linear-gradient(to right, #11998e, #38ef7d)', display: 'flex', justifyContent: 'center', alignItems: 'center'}}
        >
          {client.value === 0 && <PersonIcon style={{fontSize: '6rem', color: 'white'}}/>}
          {client.value === 1 && <BusinessIcon style={{fontSize: '6rem', color: 'white'}}/>}
        </CardMedia>
        <CardContent>
      ...
      <CardActions>
        <Button size="small" color="primary" onClick={() => setView(client.value)}>
          Nuevo
        </Button>
      </CardActions>
    </Card>
  ...
  const clients = [
    {
      ...
      description: 'Crea una cita para un cliente que no esta registrado en la empresa.',
      value: 0,
    },
    {
...
export default function MakeAnAppointment() {
  ...
  return (
    <>
      ...
      { viewRender === 2 && <><Typography variant="h4">Nueva Cita</Typography><MenuOptions setView={setView} /></> }
